...
└── selftest.ts            67 项离线断言，不联网验证全部机制
```
...
- 局限（面试可谈）：进程级隔离挡不住恶意代码读写磁盘，升级路径是 Docker/容器方案。
...
```
每轮通过率：100%  100%  95.5%   →  均值 98.5% ± 标准差 2.1%
flaky（非全过全败）任务：g01        g01  easy  pass@3=2/3(66.7%)  均回合 8.7  ⚠
其余 21 个任务 pass@3 = 3/3（含全部 10 个 hard 任务）
```

即：整体可靠性 98.5%±2.1%，**唯一的可靠性缺口被精确定位到 g01 的终止行为**——
这正是"多次跑分 + 方差 + flaky 归因"相比"单次通过率"多出来的信息量。
