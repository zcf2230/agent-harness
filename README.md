...
└── selftest.ts            97 项离线断言，不联网验证全部机制
```
...
- **判分前对 protected 验收文件做 sha256 校验**——模型即便用 run_js 绕过 write_file 拒绝改了 test.mjs，也会被检出为 `grader_tampered` 失败。selftest 由 80 → **97 项**。
...
- **文档口径**：RESUME 英文版 "83 断言" 统一为 **97**；"264 次调用" 更正为 **264 次任务运行（每次含多轮 API 调用）**。
