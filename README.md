...
> 📄 配套技术文章（含"我用 R=3 消融证伪了自己首轮结论"的完整复盘）：
> <https://juejin.cn/post/7689045619357499401>
...
└── selftest.ts            90 项离线断言，不联网验证全部机制
```
...
**可靠性曲线（`bench`）与消融（`ablate`）——首轮结论已被 R=3 重测证伪**：首轮 n=1 消融曾给出"护栏 +4.5%""压缩 −4% token"
等结论，**方法上站不住**（单轮噪声、压缩对照实为空跑、护栏效应与 g01 混淆）。于是按评审建议改成**每配置 R=3 配对**重测
（`ablate --all --runs 3`，4 配置 × 22 任务 × 3 轮 = 264 次任务运行，每次含多轮真实 API 调用），结果如下：

| 配置 | 均值% | 中位% | 最低% | 均 prompt tok | flaky |
|---|---|---|---|---|---|
| baseline（全开） | 98.5 | 100 | 95.5 | 7773 | g01 |
| 关压缩 | 97.0 | 95.5 | 95.5 | 7761 | g01 |
| 关护栏 | 97.0 | 95.5 | 95.5 | 7422 | g01 |
| 关沙箱 | 97.0 | 95.5 | 95.5 | 7251 | g01 |

> **诚实结论**：四配置全落在噪声带内（均值差 1.5%、中位几乎一致），**g01 在 baseline 里也 flaky**，"关压缩"token 与 baseline 几乎相同。
> 即在 R=3 下**没有任何机制呈现可统计分辨的通过率效应，早期单轮结论被本次重测推翻**。唯一稳定可复现的信号是：
> **开放式任务 g01 的终止 flakiness 才是本套件的真实前沿，现有护栏并不能可靠消除它。** 这份"自我证伪"数据保留在 `results/ablation.json`。
...
- **交付物兜底曾把确切文件名回灌给被测模型**（oracle 泄漏）——改为通用提醒；**g01 旧 gold 过松**——改为可执行校验（真数汉字 30–80）；
- **判分验收脚本可被模型改写**——加 `protected` 文件（write_file 拒绝覆写 + 只读）；**失败归因靠中文字符串匹配**——改结构化 reason 码；
- 其它：stderr 不再整段丢弃、同回合 `finish` 不再吞写盘、压缩自身 token 计入成本、`tsc --noEmit` 类型门禁；
- **压缩保留组改为随 token 预算缩放**（不再硬编码 keep=4）——真实跑分中 x07/x10 现能触发压缩（`compactions=1, llm`），修掉了"关压缩是空对照"的问题；
- **判分前对 protected 验收文件做 sha256 校验**——模型即便用 run_js 绕过 write_file 拒绝改了 test.mjs，也会被检出为 `grader_tampered` 失败。selftest 由 80 → **90 项**。

**第二轮评审整改（同类"声明≠实现"缺陷，均已修 + 自检锁死）**：
- **系统提示词此前把 `run_py` 也说成"受 Node 权限模型强制"**——实际只有 `run_js` 注入 `--permission`，`run_py` 无对等运行时隔离。已按解释器诚实区分（run_js 强制 / run_py 仅工具层约定 + 超时）；
- **"压缩自身 token 计入成本"在 no-benefit 分支不成立**——摘要 LLM 调用已发生、钱已花，但旧代码只在 `compacted=true` 才累计。现改为只要返回带 `usage` 就累计（含 no-benefit）；
- **`--resume` 续跑会跳过判分篡改检测**——`protectedSet`/基线哈希仅在全新运行分支设置。现续跑时按任务定义重建 protected 集并以当前文件内容作哈希基线，续跑段内改验收脚本同样判 `grader_tampered`；
- **`find_text` 把模型给的串直接 `new RegExp`**——非法正则会抛错、超长/灾难性回溯模式可拖慢主循环。现加模式长度上限与 try/catch 兜住非法正则（自检断言），真实 ReDoS 防护需换安全正则引擎（列入 Roadmap）；
- **文档口径**：RESUME 英文版 "83 断言" 统一为 **90**；"264 次调用" 更正为 **264 次任务运行（每次含多轮 API 调用）**。

> 简历叙事（诚实版）：不是"我做了个能跑分的脚本"，而是"我搭了**独立判分 + 事件溯源 + pass@R/方差/消融**的评测闭环，
> 用真实跑分修复了 4 个跨层缺陷与 3 个安全边界问题，并用 R=3 配对重测**主动证伪了自己首轮的机制结论**"——
> 能设计实验推翻自己的漂亮数字，比给一个存活的百分比更能证明工程判断力。
