...
└── selftest.ts            88 项离线断言，不联网验证全部机制
```
...
（`ablate --all --runs 3`，4×22×3=264 次真实调用），结果如下：
...
- **判分前对 protected 验收文件做 sha256 校验**——模型即便用 run_js 绕过 write_file 拒绝改了 test.mjs，也会被检出为 `grader_tampered` 失败。selftest 由 80 → **88 项**。
