...
- 零依赖 TypeScript，Node 原生运行（≥23.6），无构建、无第三方包；约 2.7k 行、17 个模块。
- **83 项离线自检断言全绿**，GitHub Actions CI（无需 API key 即可回归）。
...
- ❌ "整体可靠性 100%±0%"、"护栏 +4.5%"、"压缩省 4% token"——首轮消融为 n=1、压缩对照为空跑、g01 旧 gold 过松，
  这些数字**已被撤回**，需 R≥3 重测后才可能重新给出。
...
- 提出并实现**多次跑分可靠性度量（pass@R + 方差 + flaky 归因）与消融脚手架**；能指出首轮消融为何不成立（单轮噪声、
  空对照、gold 过松）并给出修正方案，体现评测的严谨与自我批判。
- 编写 **83 项离线自检**并配置 **GitHub Actions CI**，保证核心机制在迭代中不劣化。

## 中文 · 精简版（2 条）

- 独立实现零依赖 TS Agent Harness：OS 强制沙箱 + 凭据白名单、上下文压缩+断点续跑、独立判分、MCP 客户端；
  22 任务套件真实跑分定位并修复 4 个跨层缺陷 + 3 个安全问题。
- 设计 pass@R/方差/flaky 可靠性度量与消融脚手架；能自证首轮消融的统计缺陷并给出重测方案；83 项自检 + CI 全绿。
...
- 83 offline assertions gated by GitHub Actions CI.
