...
export function buildSystemPrompt(task: TaskDef): string {
  return [
    ...
    '- 隔离边界按解释器不同：run_js 在 Node 权限模型（--permission）下强制运行，越界文件读写会被运行时拒绝、child_process 默认禁用；run_py 仅有工具层路径约定 + 执行超时，无同等的运行时强制隔离（Python 代码可直接读写任意路径）。两者当前都不保证拦截网络；报错误码 127 表示解释器不存在；',
